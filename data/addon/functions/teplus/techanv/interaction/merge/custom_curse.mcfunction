##================================MERGE CURSE FUNCTION================================##
# This function will apply your custom curse to an enchanted book or item Output 
# in the Technical Anvil. There is a lot of stuff to cover so pay attention:
#--------------------------------------------------------------------------------------------#

#Remove ComError tag, because is a compatible curse
...
execute store result score @s[tag=TCHA.inpbk] TCHA.Section1 run data get storage teplus:tech_anvil.ui CurrentItems[{Slot:2b}].tag.StoredCustomCurse[{id:"CurseID"}].lvl
execute store result score @s[tag=TCHA.inpit] TCHA.Section1 run data get storage teplus:tech_anvil.ui CurrentItems[{Slot:2b}].tag.CustomCurse[{id:"CurseID"}].lvl
...
execute store result score @s[tag=!Is.same_item] TCHA.Section2 run data get storage teplus:tech_anvil.ui CurrentItems[{Slot:6b}].tag.StoredCustomCurse[{id:"CurseID"}].lvl
execute store result score @s[tag=Is.same_item] TCHA.Section2 run data get storage teplus:tech_anvil.ui CurrentItems[{Slot:6b}].tag.CustomCurse[{id:"CurseID"}].lvl
...
execute if entity @s[tag=TCHA.inpbk] unless data storage teplus:tech_anvil.ui ResultItem.tag.StoredCustomCurse[{id:"CurseID"}].lvl run data modify storage teplus:tech_anvil.ui ResultItem.tag.StoredCustomCurse append value {id:"CurseID",lvl:1}

#This is for other items
execute if entity @s[tag=TCHA.inpit] unless data storage teplus:tech_anvil.ui ResultItem.tag.CustomCurse[{id:"CurseID"}].lvl run data modify storage teplus:tech_anvil.ui ResultItem.tag.CustomCurse append value {id:"CurseID",lvl:1}
...
#This is for enchanted books
execute if entity @s[tag=TCHA.inpbk] store result storage teplus:tech_anvil.ui ResultItem.tag.StoredCustomCurse[{id:"CurseID"}].lvl int 1 run scoreboard players get @s TCHA.Section1

#This is for other items
execute if entity @s[tag=TCHA.inpit] store result storage teplus:tech_anvil.ui ResultItem.tag.CustomCurse[{id:"CurseID"}].lvl int 1 run scoreboard players get @s TCHA.Section1
